...
def validate_assets(assets_dir: str) -> None:
    ...
    image_assets = list(chain.from_iterable([[f'{format_path(os.path.relpath(dir, image_dir))}/{file}' for file in files] for dir, _, files in os.walk(image_dir)]))
    ref_images = set(filter(lambda e: len(e[1]) > 0, [(entry[0], format_path(entry[1].get('template') or '')) for pipeline in pipelines for entry in pipeline.items()]))
