...
public:
    ...
    using analyze_func = std::function<coro::Promise<AnalyzeResult>(
        std::shared_ptr<SyncContext>, std::shared_ptr<details::Image>, std::string_view, std::string_view)>;
...
private:
    static MaaBool run_analyze(
        ...
        auto result =
            self->analyze_(SyncContext::make(sync_context), image_handle, task_name, custom_recognition_param).sync_wait();
...
public:
    ...
    using run_func = std::function<coro::Promise<bool>(
        std::shared_ptr<SyncContext>, MaaStringView, MaaStringView, const MaaRect&, MaaStringView)>;
...
private:
    static MaaBool run_run(
        ...
        return self->run_(SyncContext::make(sync_context), task_name, custom_action_param, *cur_box, cur_rec_detail)
            .sync_wait();
    }
...
public:
    ...
    ~Instance() {
        MaaDestroy(inst_);
        for (auto action : actions_ | std::views::values) { action.lock()->wait().sync_wait(); }
    }

    std::shared_ptr<InstanceAction> post_task(const std::string& task, const json::object& param = json::object{}) {
        return put_action(MaaPostTask(inst_, task.c_str(), param.to_string().c_str()));
    }

    std::shared_ptr<InstanceAction> post_recognition(const std::string& task, const json::object& param = json::object{}) {
        return put_action(MaaPostRecognition(inst_, task.c_str(), param.to_string().c_str()));
    }

    std::shared_ptr<InstanceAction> post_action(const std::string& task, const json::object& param = json::object{}) {
    ...
    std::shared_ptr<Instance> bind(std::shared_ptr<Controller> ctrl) {
        if (!MaaBindController(inst_, ctrl ? ctrl->inst_ : nullptr)) { throw FunctionFailed("MaaBindController"); }
        controller_ = ctrl;
        return shared_from_this();
    }

    std::shared_ptr<Instance> bind(std::shared_ptr<Resource> res) {
        if (!MaaBindResource(inst_, res ? res->inst_ : nullptr)) { throw FunctionFailed("MaaBindResource"); }
        resource_ = res;
        return shared_from_this();
    }
    ...
    std::shared_ptr<Instance> bind(const std::string& name, std::shared_ptr<CustomRecognizer> reco) {
        ...
        if (!MaaRegisterCustomRecognizer(
                inst_, name.c_str(), const_cast<MaaCustomRecognizerAPI*>(&CustomRecognizer::api_), reco.get())) {
    ...
    std::shared_ptr<Instance> bind(const std::string& name, std::shared_ptr<CustomAction> act) {
        ...
        if (!MaaRegisterCustomAction(inst_, name.c_str(), const_cast<MaaCustomActionAPI*>(&CustomAction::api_), act.get())) {
    ...
    template <typename T>
        requires requires { T::name(); } && requires { T::make(); }
    std::shared_ptr<Instance> bind() {
        bind(T::name(), T::make());
        return shared_from_this();
    }
    ...
    std::shared_ptr<Instance> unbind_recognizer(const std::string& name) {
        ...
        if (!MaaUnregisterCustomRecognizer(inst_, name.c_str())) { throw FunctionFailed("MaaUnregisterCustomRecognizer"); }
        return shared_from_this();
    }

    std::shared_ptr<Instance> unbind_action(const std::string& name) {
        custom_actions_.erase(name);
        if (!MaaUnregisterCustomAction(inst_, name.c_str())) { throw FunctionFailed("MaaUnregisterCustomAction"); }
        return shared_from_this();
    }

    std::shared_ptr<Instance> unbind_recognizer() {
        custom_recognizers_.clear();
        if (!MaaClearCustomRecognizer(inst_)) { throw FunctionFailed("MaaClearCustomRecognizer"); }
        return shared_from_this();
    }

    std::shared_ptr<Instance> unbind_action() {
        custom_actions_.clear();
        if (!MaaClearCustomAction(inst_)) { throw FunctionFailed("MaaClearCustomAction"); }
        return shared_from_this();
    }

    std::shared_ptr<Instance>
        bind_recognizer_executor(const std::string& name, const std::string& path, const std::vector<std::string>& argv) {
        ...
        if (!MaaToolkitRegisterCustomRecognizerExecutor(
                inst_, name.c_str(), path.c_str(), argv_view.data(), argv_view.size())) {
    ...
    std::shared_ptr<Instance>
        bind_action_executor(const std::string& name, const std::string& path, const std::vector<std::string>& argv) {
        std::vector<MaaStringView> argv_view;
        std::transform(
            argv.begin(),
            argv.end(),
        ...
        if (!MaaToolkitRegisterCustomActionExecutor(inst_, name.c_str(), path.c_str(), argv_view.data(), argv_view.size())) {
    ...
    std::shared_ptr<Instance> unbind_action_executor() {
        if (!MaaToolkitClearCustomActionExecutor(inst_)) { throw FunctionFailed("MaaToolkitClearCustomActionExecutor"); }
        return shared_from_this();
    }

    bool inited() {
        return MaaInited(inst_);
    }

    bool running() {
        return MaaRunning(inst_);
    }

    bool stop() {
        return MaaPostStop(inst_);
    }
...
inline std::shared_ptr<InstanceAction> InstanceAction::set_param(const json::object& param) {
    if (!MaaSetTaskParam(inst_->inst_, id_, param.to_string().c_str())) { throw FunctionFailed("MaaSetTaskParam"); }
    return shared_from_this();
}
