...
public:
    ...
    Controller(
        ...
        : ActionHelper(MaaAdbControllerCreateV2(
            adb_path.c_str(), address.c_str(), type, config.c_str(), agent_path.c_str(), &Controller::_callback, this))
    ...
    Controller(
        ...
        const std::string&                                         agent_path,
        std::function<void(std::shared_ptr<message::MessageBase>)> callback = nullptr)
        : Controller({}, device.adb_path, device.address, device.type, device.config, agent_path, callback) {}
    ...
    Controller(
        ...
        Win32Type                                                  type,
        std::function<void(std::shared_ptr<message::MessageBase>)> callback = nullptr)
        : ActionHelper(MaaWin32ControllerCreate(hwnd.hwnd_, type.type_, &Controller::_callback, this))
        , user_callback_(callback) {}

    Controller(const Win32Device& device, std::function<void(std::shared_ptr<message::MessageBase>)> callback = nullptr)
        : Controller({}, device.hwnd, device.type, callback) {}

    Controller(
        std::shared_ptr<CustomControllerAPI> api, std::function<void(std::shared_ptr<message::MessageBase>)> callback = nullptr)
        : ActionHelper(
            MaaCustomControllerCreate(const_cast<MaaCustomControllerAPI*>(&api_), this, &Controller::_callback, this))
        , user_callback_(callback)
        , custom_ctrl_(api) {}
    ...
    std::shared_ptr<Controller> set_long_side(int width) {
        if (!MaaControllerSetOption(inst_, MaaCtrlOption_ScreenshotTargetLongSide, &width, sizeof(width))) {
    ...
    std::shared_ptr<Controller> set_short_side(int width) {
        if (!MaaControllerSetOption(inst_, MaaCtrlOption_ScreenshotTargetShortSide, &width, sizeof(width))) {
            throw FunctionFailed("MaaControllerSetOption");
        }
        return shared_from_this();
    }
    ...
    std::shared_ptr<Controller> set_start_entry(std::string_view entry) {
        if (!MaaControllerSetOption(
                inst_, MaaCtrlOption_DefaultAppPackageEntry, const_cast<char*>(entry.data()), entry.size())) {
            throw FunctionFailed("MaaControllerSetOption");
        }
        return shared_from_this();
    }
    ...
    std::shared_ptr<Controller> set_stop_entry(std::string_view entry) {
        if (!MaaControllerSetOption(inst_, MaaCtrlOption_DefaultAppPackage, const_cast<char*>(entry.data()), entry.size())) {
            throw FunctionFailed("MaaControllerSetOption");
        }
        return shared_from_this();
    }
    ...
    std::shared_ptr<ControllerAction> post_swipe(int32_t x1, int32_t y1, int32_t x2, int32_t y2, int32_t duration) {
    ...
    std::shared_ptr<ControllerAction> post_touch_down(int32_t contact, int32_t x, int32_t y, int32_t pressure) {
        return put_action(MaaControllerPostTouchDown(inst_, contact, x, y, pressure));
    }

    std::shared_ptr<ControllerAction> post_touch_move(int32_t contact, int32_t x, int32_t y, int32_t pressure) {
    ...
    bool connected() {
        return MaaControllerConnected(inst_);
    }
    ...
    void image(std::shared_ptr<details::Image> img) {
        if (!MaaControllerGetImage(inst_, img->handle())) { throw FunctionFailed("MaaControllerGetImage"); }
    }

    std::string uuid() {
        details::String buf;
        if (!MaaControllerGetUUID(inst_, buf.handle())) { throw FunctionFailed("MaaControllerGetUUID"); }
        return buf;
    }
...
private:
    ...
    static MaaBool _request_resolution(MaaTransparentArg handle_arg, int32_t* width, int32_t* height) {
        auto self = reinterpret_cast<Controller*>(handle_arg)->shared_from_this();
    ...
    static MaaBool _swipe(int32_t x1, int32_t y1, int32_t x2, int32_t y2, int32_t duration, MaaTransparentArg handle_arg) {
        auto self = reinterpret_cast<Controller*>(handle_arg)->shared_from_this();
        return self->custom_ctrl_->swipe(x1, y1, x2, y2, duration).sync_wait();
    }

    static MaaBool _touch_down(int32_t contact, int32_t x, int32_t y, int32_t pressure, MaaTransparentArg handle_arg) {
        auto self = reinterpret_cast<Controller*>(handle_arg)->shared_from_this();
        return self->custom_ctrl_->touch_down(contact, x, y, pressure).sync_wait();
    }

    static MaaBool _touch_move(int32_t contact, int32_t x, int32_t y, int32_t pressure, MaaTransparentArg handle_arg) {
        auto self = reinterpret_cast<Controller*>(handle_arg)->shared_from_this();
    ...
    constexpr static MaaCustomControllerAPI api_ = {
        &Controller::_connect,
        &Controller::_request_uuid,
        &Controller::_start_app,
        &Controller::_stop_app,
        &Controller::_screencap,
        &Controller::_click,
        &Controller::_swipe,
        &Controller::_touch_down,
        &Controller::_touch_move,
        &Controller::_touch_up,
