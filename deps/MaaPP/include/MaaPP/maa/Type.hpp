...
struct RecognitionDetail {
    ...
    void query(MaaRecoId reco_id) {
        ...
        MaaBool         hh;
        details::String dd;
        if (!MaaQueryRecognitionDetail(reco_id, nn.handle(), &hh, &hit_box, dd.handle(), raw.handle(), draws.handle())) {
...
struct NodeDetail {
    ...
    void query(MaaNodeId node_id) {
        MaaBool         ss;
        details::String nn;
        if (!MaaQueryNodeDetail(node_id, nn.handle(), &reco_id, &ss)) { throw FunctionFailed("MaaQueryNodeDetail"); }
        name    = nn;
        success = !!ss;
    }
};
...
struct TaskDetail {
    ...
    void query(MaaTaskId id) {
        MaaSize size;
        if (!MaaQueryTaskDetail(id, nullptr, nullptr, &size)) { throw FunctionFailed("MaaQueryTaskDetail"); }
        details::String        ee;
        std::vector<MaaNodeId> result(size);
        if (!MaaQueryTaskDetail(id, ee.handle(), result.data(), &size)) { throw FunctionFailed("MaaQueryTaskDetail"); }
        entry = ee;
    }
};
