...
inline coro::Promise<std::shared_ptr<std::vector<AdbDevice>>> find(std::optional<std::string> adb = std::nullopt) {
    if (adb.has_value()) {
        if (!MaaToolkitPostFindDeviceWithAdb(adb.value().c_str())) { throw FunctionFailed("MaaToolkitPostFindDeviceWithAdb"); }
    } else {
        if (!MaaToolkitPostFindDevice()) { throw FunctionFailed("MaaToolkitPostFindDevice"); }
    }
