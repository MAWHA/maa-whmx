...
struct ProxyController : public CustomControllerAPI {
    ...
    virtual coro::Promise<bool> swipe(int32_t x1, int32_t y1, int32_t x2, int32_t y2, int32_t duration) override {
        co_return co_await target_->post_swipe(x1, y1, x2, y2, duration)->wait() == MaaStatus_Success;
    }

    virtual coro::Promise<bool> touch_down(int32_t contact, int32_t x, int32_t y, int32_t pressure) override {
        co_return co_await target_->post_touch_down(contact, x, y, pressure)->wait() == MaaStatus_Success;
    }

    virtual coro::Promise<bool> touch_move(int32_t contact, int32_t x, int32_t y, int32_t pressure) override {
        co_return co_await target_->post_touch_move(contact, x, y, pressure)->wait() == MaaStatus_Success;
    }
