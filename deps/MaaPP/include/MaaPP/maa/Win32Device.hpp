...
struct Win32Hwnd {
    ...
    std::string class_name() const {
        ...
        if (!MaaToolkitGetWindowClassName(hwnd_, buffer.handle())) { throw FunctionFailed("MaaToolkitGetWindowClassName"); }
        return buffer;
    }

    std::string window_name() const {
        details::String buffer;
        if (!MaaToolkitGetWindowWindowName(hwnd_, buffer.handle())) { throw FunctionFailed("MaaToolkitGetWindowWindowName"); }
        return buffer;
    }
};
...
inline auto get_result(MaaSize size) {
    ...
    result.reserve(size);
    for (size_t i = 0; i < size; i++) { result.push_back({MaaToolkitGetWindow(i)}); }
    return std::make_shared<std::vector<Win32Hwnd>>(std::move(result));
}

} // namespace details

inline coro::Promise<std::shared_ptr<std::vector<Win32Hwnd>>> find(std::string class_name, std::string window_name) {
...
inline coro::Promise<std::shared_ptr<std::vector<Win32Hwnd>>> search(std::string class_name, std::string window_name) {
    auto size = co_await coro::EventLoop::current()->eval([class_name, window_name]() {
