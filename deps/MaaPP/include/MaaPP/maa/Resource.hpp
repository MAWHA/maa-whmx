...
public:
    ...
    std::shared_ptr<Resource> clear() {
        if (!MaaResourceClear(inst_)) { throw FunctionFailed("MaaResourceClear"); }
        return shared_from_this();
    }

    bool loaded() {
        return MaaResourceLoaded(inst_);
    }

    std::string hash() {
        details::String buf;
        if (!MaaResourceGetHash(inst_, buf.handle())) { throw FunctionFailed("MaaResourceGetHash"); }
        return buf;
    }

    std::shared_ptr<std::vector<std::string>> task_list() {
        details::String buf;
        if (!MaaResourceGetTaskList(inst_, buf.handle())) { throw FunctionFailed("MaaResourceGetTaskList"); }
        return std::make_shared<std::vector<std::string>>(json::parse(buf.str()).value().as<std::vector<std::string>>());
    }
