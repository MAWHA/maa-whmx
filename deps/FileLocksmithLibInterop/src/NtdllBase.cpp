...
Ntdll::Ntdll() {
    ...
    if (m_module == 0) { throw std::runtime_error{"GetModuleHandleW returned null"}; }

    m_NtQuerySystemInformation =
        reinterpret_cast<NtQuerySystemInformation_t>(GetProcAddress(m_module, "NtQuerySystemInformation"));
    ...
    if (m_NtDuplicateObject == 0) { throw std::runtime_error{"GetProcAddress returned null for NtDuplicateObject"}; }

    m_NtQueryObject = reinterpret_cast<NtQueryObject_t>(GetProcAddress(m_module, "NtQueryObject"));
    if (m_NtQueryObject == 0) { throw std::runtime_error{"GetProcAddress returned null for NtQueryObject"}; }
}
