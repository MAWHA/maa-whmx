...
std::vector<ProcessResult> find_processes_recursive(const std::vector<std::wstring>& paths) {
    ...
    for (const auto& path : paths) {
        ...
        if (!kernel_path.empty()) { (is_directory(path) ? kernel_names_dirs : kernel_names_files)[kernel_path] = path; }
    }
    ...
    auto kernel_paths_contain = [&](const std::wstring& kernel_name) -> std::wstring {
        // Normal equivalence
        if (auto it = kernel_names_files.find(kernel_name); it != kernel_names_files.end()) { return it->second; }

        if (auto it = kernel_names_dirs.find(kernel_name); it != kernel_names_dirs.end()) { return it->second; }

        for (const auto& [dir_kernel_name, dir_path] : kernel_names_dirs) {
            if (starts_with(
                    kernel_name,
                    dir_kernel_name
                        + (dir_kernel_name.length() > 0 && dir_kernel_name[dir_kernel_name.length() - 1] != L'\\' ? L"\\"
                                                                                                                  : L""))) {
    ...
    for (const auto& handle_info : nt_ext.handles()) {
        if (handle_info.type_name == L"File") {
            ...
            if (!path.empty()) { pid_files[handle_info.pid].insert(std::move(path)); }
        }
    }
    ...
    for (const auto& process : processes) {
        for (const auto& path : process.modules) {
            ...
            if (!found_path.empty()) { pid_files[process.pid].insert(std::move(found_path)); }
        }
    }
    ...
    for (const auto& process_info : processes) {
        if (auto it = pid_files.find(process_info.pid); it != pid_files.end()) {
            result.push_back(ProcessResult{
                process_info.name, process_info.pid, process_info.user, std::vector(it->second.begin(), it->second.end())});
        }
    }
