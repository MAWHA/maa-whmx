...
template <typename in_t, typename serializer_t = _serialization_helper::empty_serializer, typename string_t = default_string_t>
...
bool deserialize(const basic_value<string_t>& in, out_t& out, const deserializer_t& deserializer = {}) {
    if constexpr (_serialization_helper::is_deserializable<out_t, deserializer_t>::value) {
        ...
        for (auto&& j_elem : in.as_array()) {
            ...
            elem_t elem{};
            if (!deserialize<elem_t, deserializer_t, string_t>(j_elem, elem, deserializer)) { return false; }
        ...
        if (in_as_arr.size() != out_size) { return false; }
