...
public:
    ...
    template <
        typename map_t,
        std::enable_if_t<_utils::is_map<map_t> && std::is_constructible_v<value_type, _utils::range_value_t<map_t>>, bool> =
            true>
...
inline decltype(auto) basic_object<string_t>::emplace(args_t&&... args) {
    static_assert(
        std::is_constructible_v<value_type, args_t...>, "Parameter can't be used to construct a raw_object::value_type");
...
inline bool basic_object<string_t>::all() const {
    for (const auto& [_, val] : _object_data) {
        if (!val.template is<value_t>()) { return false; }
    }
    return true;
}
...
inline map_t<string_t, value_t> basic_object<string_t>::as_map() const {
    ...
    for (const auto& [key, val] : _object_data) { result.emplace(key, val.template as<value_t>()); }
    return result;
}
...
template <typename string_t>
template <typename value_t, typename... rest_keys_t>
inline auto basic_object<string_t>::get_helper(const value_t& default_value, const string_t& key, rest_keys_t&&... rest) const {
...
inline std::optional<value_t> basic_object<string_t>::find(const string_t& key) const {
    ...
    if (iter == _object_data.end()) { return std::nullopt; }
