...
inline bool parser5<string_t>::unicode::isIdContinueChar(u8char ch) {
    return (ch >= 'a' && ch <= 'z') || (ch >= 'A' && ch <= 'Z') || (ch >= '0' && ch <= '9') || (ch == '$') || (ch == '_')
        || findInRange(json::unicode::id_continue, toUnicode(ch));
}
...
inline void parser5<string_t>::literal(const std::string& s) {
    for (const auto& ch : s) {
        ...
        if (p != ch) { throw InvalidChar(_current_char, exceptionDetailInfo()); }
    }
}
...
inline std::optional<typename parser5<string_t>::u8char> parser5<string_t>::escape() {
    ...
    switch (c) {
        ...
        case '0':
            read();
            if (unicode::isDigit(peek(_cur, _end))) { throw InvalidChar(_current_char, exceptionDetailInfo()); }
        ...
        default:
            if (c == 0) { throw InvalidChar(_current_char, exceptionDetailInfo()); }
    }
...
inline typename parser5<string_t>::u8char parser5<string_t>::hexEscape() {
    ...
    if (!unicode::isHexDigit(c)) { throw InvalidChar(_current_char, exceptionDetailInfo()); }
...
inline std::optional<typename parser5<string_t>::Token> parser5<string_t>::lex_multiLineComment() {
    ...
    if (_current_char == 0) { throw InvalidChar(_current_char, exceptionDetailInfo()); }
...
inline std::optional<typename parser5<string_t>::Token> parser5<string_t>::lex_identifierNameStartEscape() {
    if (_current_char != 'u') { throw InvalidChar(_current_char, exceptionDetailInfo()); }
    read();
    ...
    switch (u) {
        ...
        case '_':
            break;
        default:
            if (!unicode::isIdStartChar(u)) { throw InvalidIdentifier("", exceptionDetailInfo()); }
            break;
    }
