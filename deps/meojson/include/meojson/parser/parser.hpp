...
inline std::optional<basic_value<string_t>> parser<string_t, parsing_t, accel_traits>::parse() {
    ...
    if (!result_value.valid()) { return std::nullopt; }
...
inline basic_value<string_t> parser<string_t, parsing_t, accel_traits>::parse_number() {
    ...
    if (_cur != _end && *_cur == '0' && _cur + 1 != _end && std::isdigit(*(_cur + 1))) { return invalid_value<string_t>(); }

    if (!skip_digit()) { return invalid_value<string_t>(); }
    ...
    if (*_cur == 'e' || *_cur == 'E') {
        if (++_cur == _end) { return invalid_value<string_t>(); }
        if (*_cur == '+' || *_cur == '-') { ++_cur; }
        if (!skip_digit()) { return invalid_value<string_t>(); }
    }
...
inline basic_value<string_t> parser<string_t, parsing_t, accel_traits>::parse_string() {
    ...
    if (!string_opt) { return invalid_value<string_t>(); }
...
inline basic_value<string_t> parser<string_t, parsing_t, accel_traits>::parse_array() {
    ...
    while (true) {
        if (!skip_whitespace()) { return invalid_value<string_t>(); }

        basic_value<string_t> val = parse_value();

        if (!val.valid() || !skip_whitespace()) { return invalid_value<string_t>(); }
...
inline std::optional<string_t> parser<string_t, parsing_t, accel_traits>::parse_stdstring() {
    ...
    while (_cur != _end) {
        if constexpr (sizeof(*_cur) == 1 && accel_traits::available) {
            if (!skip_string_literal_with_accel()) { return std::nullopt; }
        }
        switch (*_cur) {
            case '\t':
            case '\r':
            ...
            case '\\': {
                ...
                if (_cur == _end) { return std::nullopt; }
                if (pair_high && *_cur != 'u') { return std::nullopt; }
                switch (*_cur) {
                    case '"':
                        result.push_back('"');
                        break;
                    ...
                    case 't':
                        ...
                        break;
                    case 'u':
                        if (!skip_unicode_escape(pair_high, result)) { return std::nullopt; }
                        break;
                    default:
            ...
            case '"': {
                if (pair_high) { return std::nullopt; }
                result += string_t(no_escape_beg, _cur++);
                return result;
            }
            default:
...
template <typename string_t, typename parsing_t, typename accel_traits>
inline bool parser<string_t, parsing_t, accel_traits>::skip_unicode_escape(uint16_t& pair_high, string_t& result) {
    uint16_t cp = 0;
    for (int i = 0; i < 4; ++i) {
        if (++_cur == _end) { return false; }

        if (!std::isxdigit(static_cast<unsigned char>(*_cur))) { return false; }
...
inline bool parser<string_t, parsing_t, accel_traits>::skip_string_literal_with_accel() {
    if constexpr (sizeof(*_cur) != 1) { return false; }
...
inline bool parser<string_t, parsing_t, accel_traits>::skip_digit() {
    ...
    while (_cur != _end && std::isdigit(*_cur)) { ++_cur; }
...
auto open(const path_t& filepath, bool check_bom) {
    ...
    if (!ifs.is_open()) { return return_t(std::nullopt); }
