...
void QtMaterialFloatingActionButtonPrivate::init() {
    ...
    if (q->parentWidget()) { q->parentWidget()->installEventFilter(q); }
...
QSize QtMaterialFloatingActionButton::sizeHint() const {
    ...
    if (d->mini) {
        ...
        return QSize(
            QtMaterialFloatingActionButtonPrivate::DefaultDiameter, QtMaterialFloatingActionButtonPrivate::DefaultDiameter);
    }
}
...
bool QtMaterialFloatingActionButton::event(QEvent *event) {
    ...
    if (!parent()) { return QtMaterialRaisedButton::event(event); }
    switch (event->type()) {
