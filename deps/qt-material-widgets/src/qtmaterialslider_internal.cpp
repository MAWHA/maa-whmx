...
void QtMaterialSliderStateMachine::setupProperties() {
    ...
    if (m_slider->parentWidget()) { holeColor = m_slider->parentWidget()->palette().color(m_slider->backgroundRole()); }
...
void QtMaterialSliderThumb::paintEvent(QPaintEvent *event) {
    ...
    QRectF geometry = Qt::Horizontal == m_slider->orientation() ? QRectF(
                                                                      m_offset,
