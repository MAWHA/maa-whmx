...
QColor QtMaterialIconButton::color() const {
    ...
    if (d->useThemeColors || !d->color.isValid()) { return QtMaterialStyle::instance().themeColor("text"); }
    return d->color;
}
...
QColor QtMaterialIconButton::disabledColor() const {
    ...
    if (d->useThemeColors || !d->disabledColor.isValid()) { return QtMaterialStyle::instance().themeColor("disabled"); }
...
bool QtMaterialIconButton::event(QEvent *event) {
    ...
    switch (event->type()) {
        ...
        case QEvent::ParentChange: {
            QWidget *widget;
            if ((widget = parentWidget())) { d->rippleOverlay->setParent(widget); }
            break;
        }
        default:
            break;
