...
bool QtMaterialDrawer::event(QEvent *event) {
    ...
    switch (event->type()) {
        case QEvent::Move:
        case QEvent::Resize:
            if (!d->overlay) { setMask(QRegion(d->widget->rect())); }
            break;
        default:
            break;
    }
    ...
    switch (event->type()) {
        case QEvent::MouseButtonPress: {
            ...
            if ((mouseEvent = static_cast<QMouseEvent *>(event))) {
                ...
                if (!d->widget->geometry().contains(mouseEvent->pos()) && canClose) { closeDrawer(); }
            }
            break;
        }
        case QEvent::Move:
        case QEvent::Resize: {
            ...
            if (lw && 16 != lw->contentsMargins().right()) { lw->setContentsMargins(0, 0, 16, 0); }
            break;
        }
        default:
            break;
...
void QtMaterialDrawer::paintEvent(QPaintEvent *event) {
    ...
    if (!d->overlay || d->stateMachine->isInClosedState()) { return; }
