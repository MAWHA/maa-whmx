...
QColor QtMaterialFlatButton::foregroundColor() const {
    ...
    if (d->useThemeColors || !d->foregroundColor.isValid()) {
        if (Qt::OpaqueMode == d->bgMode) { return QtMaterialStyle::instance().themeColor("canvas"); }
        switch (d->role) {
        ...
        if (isCheckable() && progress > 0) {
            ...
            if (qFuzzyCompare(1, progress)) {
                ...
                painter->setPen(QColor(
                    COLOR_INTERPOLATE(red), COLOR_INTERPOLATE(green), COLOR_INTERPOLATE(blue), COLOR_INTERPOLATE(alpha)));
            }
        }
    } else {
